<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=rect&color=0d1117&height=1&section=header" width="100%" />
</p>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&size=22&pause=1000&color=00B4D8&center=true&vCenter=true&width=850&lines=GS-PARACOSM;MCU+%E2%86%92+EMBEDDED+LINUX+%E2%86%92+ROBOTICS;REAL-TIME+CONTROL+%C2%B7+INDUSTRIAL+BUS+%C2%B7+MOTOR+DRIVE;TRACE+FIRST+%C2%B7+PRINTF+LATER" alt="Header" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/STATUS-OPERATIONAL-00b4d8?style=flat-square&labelColor=0d1117" />
  <img src="https://img.shields.io/badge/LOC-Xi'an-8b949e?style=flat-square&labelColor=0d1117" />
  <img src="https://img.shields.io/badge/ROLE-Firmware_Engineer-7b2cbf?style=flat-square&labelColor=0d1117" />
</p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:001219,50:003049,100:0d1117&height=100&section=header&text=&fontSize=0" width="100%" />
</p>

---

## <img src="https://img.shields.io/badge/◉-00b4d8?style=flat-square&label=&labelColor=0d1117" /> 身份面板 · Identity Panel

<p align="center">
  <samp>
    <b>GS-Paracosm</b> · 嵌入式系统工程师 · 西安<br><br>
    深耕 MCU 实时控制与工业通信，正在向 Linux 与机器人底层扩展。<br>
    习惯 <code>Makefile + GCC + Shell</code> 构建烧录，也在尝试 <code>Bazel</code>。<br>
    调试信条：<b>Trace first, printf later.</b>
  </samp>
</p>

```yaml
focus:
  - MCU 高可靠实时控制
  - 工业通信 (CAN-FD / EtherCAT / Modbus)
  - 电机控制 (FOC / BLDC)
  - 机器人底层系统

toolchain:
  build   : [Makefile, CMake, Bazel(学习中)]
  compiler: [GCC ARM Embedded, Clangd]
  flash   : [OpenOCD, ST-Link CLI, J-Link, Shell脚本自动化]
  debug   : [GDB, CoreSight Trace, Segger RTT, 逻辑分析仪]
  env     : [Docker, WSL2, VSCode, Cursor]

philosophy: 嵌入式开发不能只看代码跑没跑，更要用示波器和 Trace 看到真相。
```

---

## <img src="https://img.shields.io/badge/◉-7b2cbf?style=flat-square&label=&labelColor=0d1117" /> 技术栈 · Tech Stack

<p>
  <img src="https://img.shields.io/badge/STM32H7-161b22?style=flat-square&logo=stmicroelectronics&logoColor=00b4d8&labelColor=0d1117" />
  <img src="https://img.shields.io/badge/STM32F4-161b22?style=flat-square&logo=stmicroelectronics&logoColor=00b4d8&labelColor=0d1117" />
  <img src="https://img.shields.io/badge/ARM_Cortex--M-161b22?style=flat-square&logo=arm&logoColor=c9d1d9&labelColor=0d1117" />
  <img src="https://img.shields.io/badge/FreeRTOS-161b22?style=flat-square&labelColor=0d1117&color=00b4d8" />
  <img src="https://img.shields.io/badge/RT--Thread-161b22?style=flat-square&labelColor=0d1117&color=00b4d8" />
  <img src="https://img.shields.io/badge/Zephyr-161b22?style=flat-square&logo=zephyrproject&logoColor=c9d1d9&labelColor=0d1117" />
  <img src="https://img.shields.io/badge/CMSIS--RTOS2-161b22?style=flat-square&logo=arm&logoColor=c9d1d9&labelColor=0d1117" />
</p>

<p>
  <img src="https://img.shields.io/badge/CAN--FD-161b22?style=flat-square&labelColor=0d1117&color=7b2cbf" />
  <img src="https://img.shields.io/badge/EtherCAT-161b22?style=flat-square&labelColor=0d1117&color=7b2cbf" />
  <img src="https://img.shields.io/badge/Modbus_RTU-161b22?style=flat-square&labelColor=0d1117&color=7b2cbf" />
  <img src="https://img.shields.io/badge/CANopen-161b22?style=flat-square&labelColor=0d1117&color=7b2cbf" />
  <img src="https://img.shields.io/badge/Ethernet-161b22?style=flat-square&labelColor=0d1117&color=7b2cbf" />
  <img src="https://img.shields.io/badge/RS485-161b22?style=flat-square&labelColor=0d1117&color=7b2cbf" />
</p>

<p>
  <img src="https://img.shields.io/badge/Makefile-161b22?style=flat-square&labelColor=0d1117&color=8b949e" />
  <img src="https://img.shields.io/badge/GCC-161b22?style=flat-square&logo=gnu&logoColor=c9d1d9&labelColor=0d1117" />
  <img src="https://img.shields.io/badge/Shell-161b22?style=flat-square&logo=gnu-bash&logoColor=c9d1d9&labelColor=0d1117" />
  <img src="https://img.shields.io/badge/Bazel-161b22?style=flat-square&labelColor=0d1117&color=8b949e" />
  <img src="https://img.shields.io/badge/CMake-161b22?style=flat-square&logo=cmake&logoColor=c9d1d9&labelColor=0d1117" />
  <img src="https://img.shields.io/badge/Docker-161b22?style=flat-square&logo=docker&logoColor=c9d1d9&labelColor=0d1117" />
  <img src="https://img.shields.io/badge/OpenOCD-161b22?style=flat-square&labelColor=0d1117&color=8b949e" />
  <img src="https://img.shields.io/badge/GDB-161b22?style=flat-square&labelColor=0d1117&color=8b949e" />
</p>

---

## <img src="https://img.shields.io/badge/◉-00b4d8?style=flat-square&label=&labelColor=0d1117" /> 核心能力 · Core Expertise

### ◆ Trace 调试与崩溃分析

> 遇到崩溃，先开 Trace，而不是加 `printf`。

| 能力 | 详情 |
|:---|:---|
| **Fault 分析** | HardFault / MemManage / BusFault / UsageFault 定位，从 `CFSR` `HFSR` `MMFAR` `BFAR` 还原现场 |
| **CoreSight Trace** | DWT 周期计数 · ITM 软件跟踪 · ETM 指令流追踪 |
| **Segger 工具链** | RTT 实时日志 · SystemView 任务调度可视化 |
| **栈回溯** | 通过 LR、SP、EXC_RETURN 重建调用链 |
| **Post-mortem** | 故障寄存器 · 栈内存 · Core Dump 现场还原 |

### ◆ 可靠性与固件升级
- **Bootloader / OTA**：A/B 分区 · 非活跃分区烧录 · CRC16/32 校验 · 回滚确认
- **故障保护**：IWDG · WWDG · 多级故障降级

### ◆ 工业通信
- **EtherCAT 从站**：LAN9253 PDI 中断优化 · SM 邮箱/PDO 映射 · 1ms 周期 · DWT 时序诊断
- **CAN-FD 协议**：29-bit 扩展 ID · DLC 32 控制帧 · Little-endian 线格式

---

## <img src="https://img.shields.io/badge/◉-7b2cbf?style=flat-square&label=&labelColor=0d1117" /> RTOS 矩阵 · RTOS Matrix

| RTOS | 定位 | 状态 |
|:---|:---|:---:|
| FreeRTOS | 主力实战 · Task / Queue / Semaphore / ISR Notify / PendSV / 内存管理 | ████████░░ |
| RT-Thread | 国产生态 · Device Model / FinSH / 组件化 | ████████░░ |
| CMSIS-RTOS2 | 抽象接口层 · 跨 RTOS API / Keil / STM32Cube | ███████░░░ |
| Zephyr | 现代化方向 · Device Tree / Kconfig / West | █████░░░░░ |
| ThreadX | 工业/商业 · 线程调度 / 内存池 / 低延迟 | ████░░░░░░ |
| NuttX | 类 POSIX · 文件系统 / 驱动框架 | ████░░░░░░ |

---

## <img src="https://img.shields.io/badge/◉-00b4d8?style=flat-square&label=&labelColor=0d1117" /> 项目实验室 · Project Lab

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=rect&color=161b22&height=1&section=header" width="80%" />
</p>

> <p>
>   <a href="https://github.com/ParacosmYy/PMS-PowerManagementSystem">
>     <img src="https://img.shields.io/badge/⚡_PMS_电源管理系统-0d1117?style=for-the-badge&labelColor=00b4d8&logoColor=00b4d8" />
>   </a>
> </p>
> <p>
>   <img src="https://img.shields.io/badge/STM32H7-161b22?style=flat-square&logo=stmicroelectronics&logoColor=00b4d8&labelColor=0d1117" />
>   <img src="https://img.shields.io/badge/OTA-161b22?style=flat-square&labelColor=0d1117&color=7b2cbf" />
>   <img src="https://img.shields.io/badge/A/B分区-161b22?style=flat-square&labelColor=0d1117&color=7b2cbf" />
>   <img src="https://img.shields.io/badge/ADC+DMA-161b22?style=flat-square&labelColor=0d1117&color=7b2cbf" />
>   <img src="https://img.shields.io/badge/CRC-161b22?style=flat-square&labelColor=0d1117&color=7b2cbf" />
> </p>
> <p><samp>A/B 双分区 OTA 架构，非活跃分区烧录，CRC16/32 双重校验与回滚确认。独立看门狗与多级故障保护框架。ADC + DMA 多通道采样。</samp></p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=rect&color=21262d&height=1&section=header" width="60%" />
</p>

> <p>
>   <a href="https://github.com/ParacosmYy/EtherCAT-CANFD-Gateway">
>     <img src="https://img.shields.io/badge/🌐_EtherCAT_→_CAN--FD_网关-0d1117?style=for-the-badge&labelColor=7b2cbf&logoColor=7b2cbf" />
>   </a>
> </p>
> <p>
>   <img src="https://img.shields.io/badge/STM32H7-161b22?style=flat-square&logo=stmicroelectronics&logoColor=00b4d8&labelColor=0d1117" />
>   <img src="https://img.shields.io/badge/LAN9253-161b22?style=flat-square&labelColor=0d1117&color=7b2cbf" />
>   <img src="https://img.shields.io/badge/EtherCAT_Slave-161b22?style=flat-square&labelColor=0d1117&color=7b2cbf" />
>   <img src="https://img.shields.io/badge/CAN--FD-161b22?style=flat-square&labelColor=0d1117&color=7b2cbf" />
>   <img src="https://img.shields.io/badge/1ms周期-161b22?style=flat-square&labelColor=0d1117&color=7b2cbf" />
> </p>
> <p><samp>EtherCAT 从站协议栈移植，SDO 邮箱与 PDO 周期数据映射。LAN9253 PDI 中断优化，DWT 精确时序测量与抖动诊断。</samp></p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=rect&color=21262d&height=1&section=header" width="60%" />
</p>

> <p>
>   <a href="https://github.com/ParacosmYy/DexterousHand-CANFD-Protocol">
>     <img src="https://img.shields.io/badge/🤖_灵巧手_CAN--FD_协议栈-0d1117?style=for-the-badge&labelColor=00b4d8&logoColor=00b4d8" />
>   </a>
> </p>
> <p>
>   <img src="https://img.shields.io/badge/CAN--FD-161b22?style=flat-square&labelColor=0d1117&color=7b2cbf" />
>   <img src="https://img.shields.io/badge/29--bit_ID-161b22?style=flat-square&labelColor=0d1117&color=7b2cbf" />
>   <img src="https://img.shields.io/badge/DLC32-161b22?style=flat-square&labelColor=0d1117&color=7b2cbf" />
>   <img src="https://img.shields.io/badge/Little--endian-161b22?style=flat-square&labelColor=0d1117&color=7b2cbf" />
> </p>
> <p><samp>面向灵巧手的 29-bit 扩展 CAN ID 设备寻址方案，DLC 32 综合控制帧设计，设备身份与配置协议规范，Little-endian 线格式优化。</samp></p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=rect&color=21262d&height=1&section=header" width="60%" />
</p>

> <p>
>   <a href="https://github.com/ParacosmYy/Embedded-CI-Template">
>     <img src="https://img.shields.io/badge/🛠️_嵌入式_CI_模板-0d1117?style=for-the-badge&labelColor=7b2cbf&logoColor=7b2cbf" />
>   </a>
> </p>
> <p>
>   <img src="https://img.shields.io/badge/Makefile-161b22?style=flat-square&labelColor=0d1117&color=8b949e" />
>   <img src="https://img.shields.io/badge/CMake-161b22?style=flat-square&logo=cmake&logoColor=c9d1d9&labelColor=0d1117" />
>   <img src="https://img.shields.io/badge/Docker-161b22?style=flat-square&logo=docker&logoColor=c9d1d9&labelColor=0d1117" />
>   <img src="https://img.shields.io/badge/clangd-161b22?style=flat-square&labelColor=0d1117&color=8b949e" />
> </p>
> <p><samp>Makefile / CMake 统一构建流，Docker + WSL2 跨平台工具链，VSCode / Cursor 开发环境模板，Git hooks 格式化与 CI 流水线。</samp></p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=rect&color=161b22&height=1&section=header" width="80%" />
</p>

---

## <img src="https://img.shields.io/badge/◉-7b2cbf?style=flat-square&label=&labelColor=0d1117" /> 硬件工作台 · Hardware Workbench

```text
┌─────────────────────────────────────────────────────────────────────────┐
│  🔍 DEBUG PROBE    │  J-Link ULTRA+  ·  ST-Link V3  ·  DAPLink           │
│  📡 MEASUREMENT    │  Siglent SDS1104X-E  ·  Kingst LA2016  ·  万用表      │
│  💻 TARGET BOARDS  │  STM32H743  ·  STM32F407  ·  STM32G474  ·  RPi 4     │
│  🧪 LAB SKILLS     │  SMD/QFN 焊接  ·  电源完整性分析  ·  SI 基础            │
└─────────────────────────────────────────────────────────────────────────┘
```

---

## <img src="https://img.shields.io/badge/◉-00b4d8?style=flat-square&label=&labelColor=0d1117" /> 学习路线 · Learning Roadmap

```mermaid
%%{init: {'theme': 'base', 'themeVariables': { 'primaryColor': '#001219', 'primaryTextColor': '#00b4d8', 'primaryBorderColor': '#7b2cbf', 'lineColor': '#003049', 'secondaryColor': '#0d1117', 'tertiaryColor': '#001219'}}}%%
graph LR
    A[MCU 固件与外设驱动<br/>✅ 已完成] --> B[RTOS 内核与驱动框架<br/>✅ 已完成]
    B --> C[工业通信协议<br/>✅ 已完成]
    C --> D[Bootloader / OTA / 可靠性<br/>✅ 已完成]
    D --> E[电机控制 FOC/BLDC<br/>🔄 当前阶段]
    E --> F[Embedded Linux BSP<br/>📅 计划中]
    F --> G[ROS / 机器人控制系统<br/>📅 计划中]
```

---

## <img src="https://img.shields.io/badge/◉-7b2cbf?style=flat-square&label=&labelColor=0d1117" /> 遥测数据 · Telemetry

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=ParacosmYy&theme=algolia&no-frame=true&row=1&column=7&margin-w=15" alt="Trophy" />
</p>

<p align="center">
  <img height="170" src="https://github-readme-stats.vercel.app/api?username=ParacosmYy&show_icons=true&theme=github_dark&hide_border=true&title_color=00b4d8&icon_color=7b2cbf&text_color=8b949e&bg_color=0d1117&count_private=true&include_all_commits=true" />
  <img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=ParacosmYy&layout=compact&theme=github_dark&hide_border=true&title_color=00b4d8&icon_color=7b2cbf&text_color=8b949e&bg_color=0d1117&langs_count=8" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=ParacosmYy&theme=github-dark-blue&hide_border=true&stroke=003049&ring=7b2cbf&fire=00b4d8&currStreakLabel=00b4d8&sideLabels=8b949e&dates=8b949e" alt="Streak" />
</p>

---

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,50:240046,100:001219&height=100&section=footer&text=&fontSize=0" width="100%" />
</p>

<p align="center">
  <samp>
    我目前深耕 MCU 方向，正在向嵌入式 Linux 与机器人底层控制扩展。<br>
    如果你对底层控制、异构系统、工业通信或机器人底层感兴趣，欢迎交流合作。
  </samp>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=ParacosmYy&color=00b4d8&style=flat-square&label=VIEWS" alt="Views" />
</p>
